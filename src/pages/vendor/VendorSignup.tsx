...
const resendOtp = async () => {
 ...
 const completeSignup = async () => {
    ...
    try {
       let cloudinaryUrl = '';
       if (formData.idProof) {
         cloudinaryUrl = await uploadImageCloudinary(formData.idProof);
       }
       const signupPayload = {
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              <div
                ...
                className="border border-gray-300 rounded-lg p-3 cursor-pointer flex items-center justify-between bg-gray-50 hover:bg-gray-100 transition-colors"
              >
                <span className={`text-sm ${selectedFileName ? 'text-gray-800' : 'text-gray-500'}`}>
                  {selectedFileName || "Upload Government-Issued ID Proof"}
                </span>
            ...
            <button
              disabled={isLoading}
              type="submit"
              className="w-full py-2 px-4 bg-[#f69938] text-white rounded-lg hover:bg-[#e58828] transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-[#f69938] focus:ring-opacity-50"
            >
